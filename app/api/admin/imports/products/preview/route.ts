...
export async function POST(request: Request) {
  ...
  if (!(file instanceof File)) {
    return NextResponse.json(
      { error: { code: "FILE_REQUIRED", message: "Choose an .xlsx workbook." } },
      { status: 400 }
    );
  }
  if (!file.name.toLowerCase().endsWith(".xlsx")) {
    return NextResponse.json(
      { error: { code: "INVALID_FILE_TYPE", message: "Only .xlsx workbooks are accepted." } },
      { status: 415 }
    );
  }
  if (file.size > 10 * 1024 * 1024) {
    return NextResponse.json(
      { error: { code: "FILE_TOO_LARGE", message: "The workbook must be 10 MB or smaller." } },
      { status: 413 }
    );
  }
  ...
  try {
    ...
    // Trả commitRows cho client → server action commit (chỉ valid).
